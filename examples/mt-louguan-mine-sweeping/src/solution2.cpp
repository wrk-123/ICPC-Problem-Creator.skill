...
int main() {
    ...
    vector<vector<vector<int>>> add_cost(n, vector<vector<int>>(n + 1, vector<int>(n + 1, 0)));
    ...
    for (int row = 0; row < n; ++row) {
        ...
        for (int used = 0; used <= mines; ++used) {
            for (int prev = 0; prev <= n; ++prev) {
                int base = dp[used][prev];
                if (base == INF) {
                    continue;
                }
                int limit = min(row == 0 ? n : prev, mines - used);
                int cost_prev = row == 0 ? n : prev;
                for (int cur = 0; cur <= limit; ++cur) {
                    next_dp[used + cur][cur] = min(next_dp[used + cur][cur], base + add_cost[row][cost_prev][cur]);
                }
            }
        }
        dp.swap(next_dp);
