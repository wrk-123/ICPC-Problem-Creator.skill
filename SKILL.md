...
如果无法启动子 agent，则需阅读这三个 `SKILL.md`，串行完成这些操作，以免完成的结果与预期不符，造成返工。
...
- 样例解释的职责是帮助读者理解输入输出与过程，不是提前泄露关键结论、核心公式、主要性质或标准做法；如果读者可以直接通过画图、手算或代入定义看懂样例，就可以不写样例解释。
- 应该认真判断是否真的需要样例解释；需要时再写，而且应避免把解题关键直接写进样例解释里。
